...
Thus there needs to be a way for the [optimiser](../optimisation/Model_Optimisation_Design.md) to call this function regardless of the language of either.
...
The **MCR** is a platform-dependent installation (approximately 1GB download) which contains the base Matlab distribution and libraries set up to run only code from `ctf`s.
...
This also means that data arrays need not be copied between Python and Matlab, but instead require only a [thin "converter" wrapper](02_pace_python_implementation_discussion.md#type_conversions).
That is, Matlab data arrays may be converted to `numpy` arrays by creating a `numpy` wrapper which points to the memory of the Matlab data array without copying the data, and vice versa.
Care must be taken to ensure that we always use a (Fortran-style) column-major data layout (`numpy` supports both column- and row-major layouts but Matlab only supports the former).
...
By default scalar data types passed between Matlab and Python are automatically converted as [detailed here](https://www.mathworks.com/help/matlab/matlab_external/handle-data-returned-from-matlab-to-python.html).
The exception to this is a `matlab.object` which may only be generated by a Matlab function and is passed directly to Python.
...
As shown in [this section](02_pace_python_implementation_discussion.md#type_conversions), these "conversions" may be done without copying the underlying arrays by wrapping the array memory in the appropriate (Matlab or `numpy`) objects.

Data types which are not basic scalar types or arrays of such, including Python functions and objects are not allowed to be passed to Matlab, and will raise error if tried.
...
Finally, whilst the [prototype implementation](02_pace_python_implementation_discussion.md#user_interface) does not yet support a user friendly, "Pythonic", syntax it is envisaged that syntax like:

```python
import pyhorace as h
proj = h.projaxes(u=[ -0.5, 1, 0], v=[0, 0, 1], type='rrr')
w1 = h.cut_sqw('ei30_10K.sqw', proj, [0.1, 0.02, 0.5], [1.5, 2.5], [0.4, 0.5], [3, 0.5, 20])
w2 = w1.cut([0.1, 0.5], [3, 0.5, 20])  # or w2 = h.cut_sqw(w1, [0.1, 0.5], [3, 0.5, 20])
h.plot(w2)  # or w1.plot()
```
...
Depending on the outcome of the design review for the optimiser, described [here](../optimisation/Model_Optimisation_Design.md), the optimiser may be written in Matlab or in Python (or perhaps C++).

If the optimiser is written in Matlab (as is currently the case), then for the Python interface it needs to be able to call a Python user defined model function (because we cannot define a Matlab function using a compiled Matlab library).
...
However, the built-in Matlab-Python interface does not allow Python functions (or other objects) to be passed to Matlab.
...
An implementation of the second method might use a Python module (say `MatCallPy`) which is loaded at the same time as the Matlab interpreter and defines a list of valid Python function objects.
...
The wrapper first adds the user defined model function to the (global) `MatCallPy` list of valid functions.
...
The Matlab optimiser then calls a `mex` file with appropriate Python C API calls which checks the `MatCallPy` list, obtains the Python function from its index and calls it.
...
There is also a [reference implementation](02_pace_python_implementation_discussion.md#matpymat) for this use-case.
...
The implementations described above rely on Matlab and Python interpreters being loaded into the same process and thus sharing memory.
...
2. In the most common use case, both Matlab and Python interpreters are loaded into the same process so can read each others data directly without copying.
...
However, as this is progressing, it would likely introduce a lot of complications into the build system and testing framework, which might mean that the code becomes less stable.
...
It is dependent on being able to access the Matlab Compiler Toolbox which is "free" for us for the moment, but may not be in future (whenever the licenses are renegotiated).
